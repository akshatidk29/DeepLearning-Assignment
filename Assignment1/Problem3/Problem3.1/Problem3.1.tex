...
\begin{enumerate}
    ...
    \item \textbf{Training Time:} The average time per epoch was approximately \textbf{6.52 seconds} (excluding the first epoch).
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Training Time:} The average time per epoch was approximately \textbf{6.51 seconds}. Although ReLU is computationally cheaper than Sigmoid (avoiding expensive exponential operations), the addition of Batch Normalization layers in Experiment B introduces extra overhead (mean/variance calculation and normalization steps). These factors roughly cancel out, resulting in similar epoch times.
\end{enumerate}
...
The models were evaluated on the held-out test set from Tiny ImageNet (10 classes).

\textbf{Results:}
\begin{itemize}
    \item \textbf{Experiment A (Sigmoid):} Test Accuracy = \textbf{25.20\%}
    \item \textbf{Experiment B (Leaky ReLU + BN):} Test Accuracy = \textbf{41.20\%}
\end{itemize}

The test results closely mirror the validation performance, confirming that the Leaky ReLU + BN model generalizes significantly better than the Sigmoid baseline, despite the observed overfitting during training.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/test_confusion_matrix.png}
    \caption{Confusion Matrix for Experiment B (Leaky ReLU + BN) on the Test Set.}
